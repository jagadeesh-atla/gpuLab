...
static void h_nbody_update_velocity(real *px, real *py, real *vx, real *vy,
                                    real *ax, real *ay, int N,
                                    int *exceeded_speed, int id) {
  real total_ax = 0.0f;
  real total_ay = 0.0f;
  ...
  if (v > MAX_SPEED) *exceeded_speed = *exceeded_speed + 1;
}

static void h_nbody_update_position(real *px, real *py, real *vx, real *vy,
  ...
  if (dist > MAX_DIST) *beyond_bounds = 1;
}

#endif  // VALIDATE

/**
 * CUDA implementation of single NBody
 */
__global__ void d_nbody_update_velocity(real *px, real *py, real *vx, real *vy,
                                        real *ax, real *ay, int N,
                                        int *exceeded_speed) {
  ...
  if (v > MAX_SPEED) atomicAdd(exceeded_speed, 1);
}

__global__ void d_nbody_update_position(real *px, real *py, real *vx, real *vy,
...
int main(int argc, char **argv) {
  ...
  CHECK(cudaMalloc(&d_px, N * sizeof(real)));
  CHECK(cudaMalloc(&d_py, N * sizeof(real)));

  CHECK(cudaMalloc(&d_vx, N * sizeof(real)));
  CHECK(cudaMalloc(&d_vy, N * sizeof(real)));

  CHECK(cudaMalloc(&d_ax, N * sizeof(real)));
  CHECK(cudaMalloc(&d_ay, N * sizeof(real)));

  CHECK(cudaMalloc(&d_exceeded_speed, sizeof(int)));
  CHECK(cudaMalloc(&d_beyond_bounds, sizeof(int)));
  ...
  for (iter = 0; iter < niters; ++iter) {
    ...
    d_nbody_update_velocity<<<N / block, block>>>(d_px, d_py, d_vx, d_vy, d_ax,
                                                  d_ay, N, d_exceeded_speed);
    d_nbody_update_position<<<N / block, block>>>(d_px, d_py, d_vx, d_vy, N,
                                                  d_beyond_bounds);
  }
...
#pragma omp parallel for
    for (id = 0; id < N; ++id) {
      h_nbody_update_velocity(host_px, host_py, host_vx, host_vy, host_ax,
                              host_ay, N, &host_exceeded_speed, id);
    }
#pragma omp parallel for
    for (id = 0; id < N; ++id) {
      h_nbody_update_position(host_px, host_py, host_vx, host_vy, N,
                              &host_exceeded_speed, id);
    }
  }
#endif  // VALIDATE
  ...
  for (i = 0; i < N; i++) {
    double dist =
        sqrt(pow(h_px[i] - host_px[i], 2.0) + pow(h_py[i] - host_py[i], 2.0));
    error += dist;
  }
  error /= N;
  printf("Error = %.20e\n", error);
